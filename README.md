...
To setup a dask cluser first start a scheduler, the scheduler address will be printed to the terminal
```
dask scheduler
```
Now to start workers accross multiple nodes, simply SSH to a connected node and run, repeat this for each node you wish to add to the cluster (ensure the conda environment and any other dependencies are loaded as you would normally run MolScore)
```
