                ...
                with st.container():
                    ...
                    for j, (m, column2) in enumerate(zip(cluster['members'], cycle(st.columns(5)))):
                        ...
                        if SS.pymol is not None:
                            show_pymol = column2.button(label='Send2PyMol', key=f'{m}_pymol_button')
                            if show_pymol:
                                match_idx = main_df.loc[(main_df.run == os.path.basename(input_dir)) & (main_df.smiles == m)].drop_duplicates().index[0]
                                file_paths, names = utils.find_sdfs([match_idx], main_df)
                                utils.send2pymol(name=names[0], path=file_paths[0], pymol=SS.pymol)

                    # Send all to pymol
                    if SS.pymol is not None:
                        if st.button(f'SendAll2Pymol', key=f'AllPyMol_{os.path.basename(input_dir)}'):
                            match_idxs = list(main_df.loc[(main_df.run == os.path.basename(input_dir)) & (main_df.smiles.isin(cluster['members']))].drop_duplicates().index)
            ...
            with st.expander(label='Export selected clusters'):
                ...
                if st.button(label='Save', key='save_all_clusters'):
                    ...
                    for cluster in memory_list[:show_no]:
                        match_idx = list(main_df.loc[(main_df.run == os.path.basename(input_dir)) & (main_df.smiles.isin(cluster['members']))].drop_duplicates().index)
                        paths, names = utils.find_sdfs(match_idx, main_df)
                        file_paths += paths
                        mol_names += [f'ClusterCentroid: {names[0].split(":")[1]} - {n}' for n in names]
                    utils.save_sdf(
