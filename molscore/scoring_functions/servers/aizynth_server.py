"""
Interface with AiZynthfinder https://github.com/MolecularAI/aizynthfinder through a Flask server.
"""
...
class Model:
    ...
    def __init__(
        ...
        C: float = 1.4,
        custom_smiles: Optional[os.PathLike] = None,  # TODO load SMILES
    ):
        """
        ...
        if custom_smiles is not None:
            custom_smiles = read_smiles(custom_smiles)
    ...
    def score(self, smi: str) -> dict[str, Any]:
        """
        ...
        # img = RouteImageFactory(yaml.safe_load(r['route_json'])).image

        return {
            "smiles": smi,
            "is_solved": results["is_solved"],
...
def get_args():
    ...
    # AiZynthFinder configuration arguments
    parser.add_argument(
        "--iteration_limit",
        type=int,
        default=100,
        help="The maximum number of iterations for the tree search",
    )
    parser.add_argument(
        "--time_limit",
        type=int,
        default=120,
        help="The maximum number of seconds to complete the tree search",
    )
    parser.add_argument(
        "--return_first",
        action="store_true",
        help="If true, the tree search will be terminated as soon as one solution is found",
    )
    parser.add_argument(
        "--max_transforms",
        type=int,
        default=6,
        help="The maximum depth of the search tree",
    )
    parser.add_argument(
        "--exclude_target_from_stock",
        action="store_true",
        help="If the target is in stock it will be broken down if this property is True",
    )
    parser.add_argument(
        "--use_rdchiral",
        action="store_true",
        help="If true, will apply templates with RDChiral, otherwise RDKit will be used",
    )
    parser.add_argument(
        "--cutoff_cumulative",
        type=float,
        default=0.995,
        help="The accumulative probability of the suggested templates is capped at this value, all other templates above this threshold are discarded",
    )
    parser.add_argument(
        "--cutoff_number",
        type=int,
        default=50,
        help="The maximum number of templates that will be returned from the expansion policy",
    )
    parser.add_argument(
        "--C",
        type=float,
        default=1.4,
        help="The C value used to balance exploitation and exploration in the upper confidence bound score of the nodes",
    )
    parser.add_argument(
        "--custom_smiles",
        type=os.PathLike,
        help="List of custom SMILES to add as available stock building blocks",
    )
