...
def compute():
    ...
    for i, smi in enumerate(smiles):
        r = {'smiles': smi}
        r.update({f'pred{pi+1}': 0.0 if p == 'Invalid SMILES' else p for pi, p in enumerate(preds[i])})
        r.update({
        ...
        r.update({f'unc{ui+1}': 0.0 if u == 'Invalid SMILES' else u for ui, u in enumerate(uncs[i])})
        r.update({
