...
from hsr.utils import PROTON_FEATURES, extract_formal_charge, extract_proton_number
...
>>>>>>> a825710b27716bd3b98e35b59520136bad644d02
        ...
        if isinstance(ref_molecule, str):
            if os.path.isabs(ref_molecule):
                ...
                self.ref_molecule_path = os.path.join(
                    MOLSCORE_PATH, ref_molecule
        ...
        if isinstance(ref_molecule, str) and os.path.exists(self.ref_molecule_path):
            if self.ref_molecule_path.endswith(".txt"):
                self._initialize_from_txt(
                    self.ref_molecule_path
                )  # Handle Benchmark case
            else:
                self._initialize_from_3d_file(
                    self.ref_molecule_path
                )  # Handle 3D molecule file
        ...
        elif isinstance(ref_molecule, str) and not os.path.exists(
            self.ref_molecule_path
        ):
    ...
    def _initialize_from_txt(self, txt_file):
        ...
        if self.generator == "ccdc":
            ref_mol = component_of_interest(
                self.io.EntryReader("CSD").entry(csd_entry).molecule
            )
    ...
    def _initialize_from_3d_file(self, file_path):
        ...
        if self.generator == "ccdc":
            ...
            ref_mol_array = fp.generate_fingerprint_from_molecule(
                ref_mol, self.feature_set
            )
        ...
        try:
            ...
            if isinstance(mol, str):
                ...
                if isinstance(molecule, pb.Molecule):
                    ...
                    mol_fp = fp.generate_fingerprint_from_molecule(
                        molecule, self.feature_set
                    )
            ...
            # The molecule is a pybel molecule (obabel)
                ...
                result = {"molecule": mol}
                result.update({"3d_mol": mol.write("sdf")})
            ...
            result.update(
                {"molecule": mol, "3d_mol": 0.0, f"{self.prefix}_HSR_score": 0.0}
            )
            return result
    ...
    def score(self, molecules: list, directory, file_names, **kwargs):
        ...
        directory = os.path.join(os.path.abspath(directory), f"{self.prefix}_HSR", step)
...
<<<<<<< HEAD
        ...
        all_ccdc = ccdc_available and all(
            isinstance(mol, self.ccdcMolecule) for mol in molecules
        )
    ...
    def __call__(self, directory, file_names, smiles=None, **kwargs):
        ...
        if smiles is None:
            ...
            if molecules is None:
                raise ValueError(
                    "Either 'smiles' or 'molecules' in kwargs must be provided."
                )
        else:
            molecules = smiles
