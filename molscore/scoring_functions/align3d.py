...
class Align3D:
    ...
    def __init__(
        ...
        # Check input
        assert (len(self.ref_smiles) > 0) or (self.ref_sdf is not None), "Must specify reference smiles or SDF file"
        if self.ref_sdf: assert os.path.exists(ref_sdf), "SDF file not found"
        ...
        if self.ref_sdf:
            ...
            for i, mol in enumerate(supplier):
                if mol:
                    if mol.GetNumConformers() == 1:
                        ...
                        logger.warning(f"Molecule {i} from {os.path.basename(self.ref_sdf)} has no conformation")
                        mol = self.prepare_mol(mol)
                        to_align.append(mol)
                else:
                    logger.warning(f"Failed to load molecule {i} from {os.path.basename(self.ref_sdf)}")
            ...
            if len(self.ref_mols) > 0:
                ...
                # Align to each other
                logger.warning(f"Optimizing starting conformation, this may take several minutes")
            ...
            if mol:
                ...
                for conf_id in range(mol.GetNumConformers()):
                    ...
                    for omol in mols:
                        ...
                        _, (_, score) = self.align_best_conf(mol=omol, ref_mol=mol, ref_conf_id=conf_id)
    ...
    @staticmethod
    def align_best_conf(mol: Chem.rdchem.Mol, ref_mol: Chem.rdchem.Mol, ref_conf_id: int = -1) -> Tuple[Chem.rdchem.Mol, Tuple[int, float]]:
    ...
    @staticmethod
    def align_joint_best_conf(mol: Chem.rdchem.Mol, ref_mol: Chem.rdchem.Mol) -> Tuple[int, int, float]:
        ...
        for ref_conf_id in range(ref_mol.GetNumConformers()):
            res = Chem.GetO3AForProbeConfs(prbMol=mol, refMol=ref_mol, refCid=ref_conf_id, maxIters=100)
        ...
        res = Chem.GetO3A(prbMol=mol, refMol=ref_mol, prbCid=best_score[0], refCid=best_score[1], maxIters=100)
        res.Align()
        return mol, ref_mol, best_score  # mol, ref_mol, (prb_conf_id, ref_conf_id, score)
    ...
    def get_shape_similarity(mol1, mol2, mol1_cid=-1, mol2_cid=-1, method='Tanimoto'):
        if method == 'Tversky':
            sim = Chem.ShapeTverskyIndex(mol1, mol2, alpha=0.5, beta=0.5, confId1=mol1_cid, confId2=mol2_cid)
        else:
            sim = (1 - Chem.ShapeTanimotoDist(mol1, mol2, confId1=mol1_cid, confId2=mol2_cid))
        return sim

    @ staticmethod
    def get_3D_pharmacophore_fp(mol: Chem.rdchem.Mol, conf_id=-1):
        fp = Generate.Gen2DFingerprint(mol, Gobbi_Pharm2D.factory, dMat=Chem.Get3DDistanceMatrix(mol, confId=conf_id))
        return fp
    ...
    @staticmethod
    def score_smi(smi: str, prefix: str, ref_mols: list, ref_fps: list, max_confs: int, similarity_method: str, agg_method, pharmacophore: bool):
        result = {'smiles': smi}
        mol = Chem.MolFromSmiles(smi)
        if mol:
            # Prepare confs
            ...
            for rmol, rfp in zip(ref_mols, ref_fps):
                ...
                cids.append(cid)
                sim_scores.append(Align3D.get_shape_similarity(mol1=mol, mol2=rmol, mol1_cid=cid, method=similarity_method))
                fsim_scores.append(Align3D.get_pharmacophore_similarity(Align3D.get_3D_pharmacophore_fp(mol, conf_id=cid), rfp))
            # Align mol to most similar rfp
            best_ref_idx = int(np.argmax(O3A_scores))
            best_cid_idx = cids[best_ref_idx]
            algn = Chem.GetO3A(prbMol=mol, refMol=ref_mols[best_ref_idx], maxIters=100, prbCid=best_cid_idx)
            algn.Align()
            ...
            if pharmacophore:
                result.update({f'{prefix}_shape_score': agg_method(sim_scores) + agg_method(fsim_scores)})
            else:
    ...
    def score(self, smiles: list, directory, file_names, **kwargs):
        ...
        pfunc = partial(
            self.score_smi, prefix=self.prefix, ref_mols=self.ref_mols, ref_fps=self.ref_fps, max_confs=self.max_confs,
            similarity_method=self.similarity_method, agg_method=self.agg_method, pharmacophore=self.pharmacophore_similarity
            )
