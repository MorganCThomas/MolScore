...
class PLANTSDock:
    ...
    def __init__(self, prefix: str, receptor, ref_ligand,  cluster: Union[str, int] = None,
        ...
        if ref_ligand.endswith('.pdb') or ref_ligand.endswith('.sdf'):
            ...
            mol2_ligand = receptor.replace(ext, '.mol2')
            self.subprocess.run(cmd=f"obabel {ref_ligand} -O {mol2_ligand} --partialcharge none")
            ref_ligand = mol2_ligand

        # Specify class attributes
        self.prefix = prefix.replace(" ", "_")
        self.plants_metrics = PLANTSDock.return_metrics
    ...
    def run_plants(self):
        """
        Write PLANTS new input files and submit each to Glide
        """
        plants_commands = []
                    ...
                    try:
                        ...
                        if (best_score[name] is None) or (dscore < best_score[name]):
                            ...
                            try:
                                ...
                                docking_result.update({f'{self.prefix}_' + k: 0.0 for k in ['NetCharge', 'PositiveCharge', 'NegativeCharge']})
                                logger.debug(f'Error calculating charge for {name}-{variant}: {e}')                                  
            ...
            if len(files) > 0:
                try:
                    files = [file for file in files
                                if not ("log.txt" in file) and not any([p in file for p in keep_files])]
                    dirs = [d for d in files if os.path.isdir(d) and not any([p in d for p in keep_dirs])]

                    if parallel:
                        [del_files.append(file) for file in files if not os.path.isdir(file)]
                        [del_dirs.append(d) for d in dirs]
                    else:
                        #logger.debug(f'Will remove these files: \n{[file for file in files if not os.path.isdir(file)]}')
