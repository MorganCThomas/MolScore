...
from molscore.scoring_functions.utils import Pool, read_smiles, canonize_smiles
...
class BloomFilter:
    ...
    return_metrics = ['inside', 'outside'] # Name of metrics returned so that they can be selected in the config GUI
    
    def __init__(self,
    prefix: str, preset: str = None, bloom_path: Union[str, os.PathLike] = None, smiles_path: Union[str, os.PathLike] = None, 
    canonize = True, fpr: float = 0.01, n_jobs: int = 1, **kwargs
    ):
        """
        ...
        parameters_provided = sum([p is not None for p in [self.preset, self.smiles_path, self.bloom_path]])
        if parameters_provided > 1:
            logger.warning(f"Multiple parameters provided, precedent for use is preset first, then bloom_path, then smiles_path")

        if self.preset:
            # Download if not downloaded
            assert self.preset in molbloom.catalogs(), f"Preset {self.preset} not found in MolBloom catalog"
            ...
            if self.canonize:
                with Pool(self.n_jobs) as pool:
                    smiles_list = [s for s in pool.map(canonize_smiles, smiles_list)]
            ...
            raise ValueError("At least one of preset, smiles_path or bloom_path must be provided")
