...
class SubstructureFilters:
    ...
    def __init__(self, prefix: str, az_filters: bool = False, mcf_filters: bool = False, pains_filters: bool = False, custom_filters: list = [],
                 n_jobs: int = 1, **kwargs):
        """
        ...
        (https://github.com/MolecularAI/Reinvent)
        :param mcf_filters: Run medicinal chemistry filters (MCF)
        :param pains_filters: Run PAINS filters
        ...
        if az_filters:
            self.smarts += self.az_smarts
        if mcf_filters:
            self.smarts += self.mcf_smarts
        if pains_filters:
            self.smarts += self.pains_smarts
        if len(custom_filters) > 0:
            self.smarts += custom_filters

        # Check all SMARTS are parse-able
        self.smarts = [sub for sub in set(self.smarts) if Chem.MolFromSmarts(sub)]
    ...
    def match_substructure(smi: str, smarts_filters: list):
        ...
        if mol:
            matches = [mol.HasSubstructMatch(Chem.MolFromSmarts(sub)) for sub in smarts_filters]
            match = any(matches)
            if match:
                    subs = " ".join([smarts_filters[i] for i in np.argwhere(matches).flatten()])
            else:
                subs = None
            score = int(not match) 
        else:
    ...
    def __call__(self, smiles: list, **kwargs):
        ...
        with Pool(self.n_jobs) as pool:
            ...
            results = [{'smiles': smi, f'{self.prefix}_substruct_filt': match, f'{self.prefix}_substruct': subs}
