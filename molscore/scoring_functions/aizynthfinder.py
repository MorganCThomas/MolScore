...
class AiZynthFinder:
    ...
    def __init__(self, 
        ...
        def try_resources(input):
            try:
                with resources.path('molscore.data.models.aizynth', resource=f'{input}.hdf5') as r:
                    output = str(r)
                    return output
        ...
        if (config_file is None) or (config_file in ['.', 'None']):
            ...
            if filter_policy and (filter_policy != 'None'):
                ...
                self.config.update({
                    'filter': {
                        'files': {
                            'my_policy': try_resources(filter_policy)
                        }
                    }
                })
    ...
    def score(self, smiles, directory, **kwargs):
        ...
        for i, out in zip(valid, output_data):
            results[i].update({f"{self.prefix}_{metric}": out[metric] for metric in self.return_metrics})
            # Also add precursors
