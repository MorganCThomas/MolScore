...
from openeye import oechem, oeomega, oedocking
...
class OEDock:
    ...
    def load_prepared_ligand(self) -> oechem.OEMol:
        ...
        for name in self.variants:
            for variant in self.variants[name]:
                file_path = os.path.join(self.directory, f'{name}-{variant}_prepared.sdf')
    ...
    @staticmethod
    def omega(mol: oechem.OEMol, omega_energy_window, omega_max_confs, omega_strict_stereo) -> oechem.OEMol:
        """
    ...
    @staticmethod
    def dock(mol: oechem.OEMol, receptor, dock_scoring_function, dock_search_resolution, num_poses) -> oechem.OEMol:
        """
        Docking multi conformer mols using oedock
        """
        # Setup dock in thread (object not pickle-able)
        dockOpts = oedocking.OEDockOptions()
        dockOpts.SetScoreMethod(getattr(oedocking, f"OEDockMethod_{dock_scoring_function}"))
        dockOpts.SetResolution(getattr(oedocking, f"OESearchResolution_{dock_search_resolution}"))   
        ...
        # Save data
        dockedMol.AddData(oechem.OEGetTag('name'), mol.GetData('name')) # Copy over name
        dockedMol.AddData(oechem.OEGetTag('variant'), mol.GetData('variant')) # Copy over variant
        oechem.OESetSDData(dockedMol, 'name', f"{mol.GetData('name')}-{mol.GetData('variant')}") # Copy name to SD tag
    ...
    def get_docking_scores(self, smiles: list, return_best_variant: bool = False):
        ...
        """
        # Read in docked file
        results = {name: dict(smiles=smi, **{f'{self.prefix}_{m}': 0.0 for m in self.return_metrics}) for smi, name in zip(smiles, self.file_names)}
        ...
        for i, mol in enumerate(self.mols):
            ...
            if best_score[name] is None:
                ...
                net_charge, positive_charge, negative_charge = MolecularDescriptors.charge_counts(mol_smiles)
                results[name].update({f'{self.prefix}_NetCharge': net_charge,
        ...
        for smi, name in zip(smiles, self.file_names):
            ...
            ofs = oechem.oemolostream(os.path.join(self.directory, f'{best_variant}_docked.sdf'))
