...
class VinaDock:
    ...
    def __init__(self, prefix: str, receptor: Union[str, os.PathLike], ref_ligand: Union[str, os.PathLike],
                 ...
                 dock_scoring: str = 'vina', dock_timeout: float = 120.0, **kwargs):
        """
        ...
        if self.prep_env == 'mgltools':
            assert 'mgltools' in os.environ.keys(), "Can't find mgltools in PATH, please export the path the mgtltools 1.5.6 as \'mgltools\'"
            self.prep_env = os.path.join(os.environ['mgltools'], 'bin', 'pythonsh')
            self.prep_rec = os.path.join(os.environ['mgltools'], 'MGLToolsPckgs/AutoDockTools/Utilities24', 'prepare_receptor4.py')
            self.prep_lig = os.path.join(os.environ['mgltools'], 'MGLToolsPckgs/AutoDockTools/Utilities24', 'prepare_ligand4.py')
        ...
        if not receptor.endswith('.pdbqt'):
            ...
            if self.prep_env == 'obabel':
                out, err = self.subprocess.run(" ".join([self.prep_env, receptor, "-O", pdbqt_receptor]))
            else:
                out, err = self.subprocess.run(" ".join([self.prep_env, self.prep_rec, "-r", receptor, "-o", pdbqt_receptor, "-A", "checkhydrogens"]))
        ...
        if lig_mol:
            ...
            raise ValueError("Error parsing reference ligand, cannot calculate docking box")

        # If ligand is not pdbqt, convert
        if not ref_ligand.endswith('.pdbqt'):
            pdbqt_ligand = ref_ligand.rsplit(".")[0] + ".pdbqt"
            if self.prep_env == 'obabel':
                out, err = self.subprocess.run(" ".join([self.prep_env, ref_ligand, "-O", pdbqt_ligand]))
            else:
                # Prepare_ligand4 only takes pdb or mol2, so convert first
                if not ref_ligand.endswith('.pdb'):
                    out, err = self.subprocess.run(" ".join(["obabel", ref_ligand, "-O", ref_ligand.rsplit(".")[0] + ".pdb"]))
                    ref_ligand = ref_ligand.rsplit(".")[0] + ".pdb"
                out, err = self.subprocess.run(" ".join([self.prep_env, self.prep_lig, "-l", ref_ligand, "-o", pdbqt_ligand]))
    ...
    def ligands2pdbqt(self, variants, variant_files):
        ...
        if self.client:
            ...
            if cmds1: _ = [p(cmd) for cmd in cmds1]
    ...
    def dock_ligands(self, variant_paths):
        vina_commands = []
        log_paths = []
        for l in variant_paths:
            out_file = os.path.join(self.directory, os.path.basename(l).replace("_prepared.pdbqt", "_docked.pdbqt"))
            cmd = f"{self.vina_env} --receptor {self.receptor} --ligand {l} --out {out_file} " \
            ...
            for variant in self.variants[name]:
                ...
                if os.path.exists(out_file):
                    try:
                        ...
                        mol = manually_charge_mol(Chem.AddHs(mol)) # TODO incorrectly computing charge, RDKit bug doesn't add Hs
                        dscore, interscore, intrascore = self.parse_vina_score(mol.GetPropsAsDict()['REMARK'])
