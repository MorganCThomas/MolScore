...
class GninaDock:
    ...
    def __init__(self, prefix: str, receptor: Union[str, os.PathLike], ref_ligand: Union[str, os.PathLike],
                 ...
                 dock_timeout: float = 180.0, gpu_device: int = -1, **kwargs):
        """
        ...
        if self.gpu:
            ...
            if gpu_device >= self.ndevice:
                warnings.warn(f"GPU index selected {gpu_device} is more than the number of devices recognised by cuda {self.ndevice}", ResourceWarning)
    ...
    def dock_ligands(self, ligand_paths):
        ...
        for l in ligand_paths:
            ...
            if self.gpu:
                ...
                device = f'--no_gpu'
            cmd = f"{self.gnina_env} -r {self.receptor} -l {l} --autobox_ligand {self.ref} -o {out_file} " \
                ...
                try:
                    ...
                    if dscores is not None:
                        ...
                        if best_score[name] is None:
                            ...
                            logger.debug(f'Docking score for {name}-{variant}: {dscores[2]}')
