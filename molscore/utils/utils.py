...
def non_dominated_sort(X: np.ndarray) -> list:
    ...
    count = np.zeros(len(X), dtype=int) # Count of how many times this solution dominates another
    ...
    for i, front in enumerate(fronts):
        ...
        if (len(front) > 2) and None not in fp:
            ...
            for j in range(len(front)):
                tanimoto = 1 - np.array(SimilarityMeasures.get('Tanimoto', bulk=True)(fp[j], fp))
