"""Functions that can be used to preprocess SMILES sequences in the form used in the publication."""
import numpy as np
import pandas as pd
...
def dataframe_to_tfrecord(df,
                          ...
                          shuffle_first=False):
    """Function to create a tf-record file to train the translation model from a pandas dataframe.
    Args:
...
def create_feature_df(smiles_df, n_jobs=1):
    with Pool(n_jobs) as pool:
        temp = list(zip(*[desc for desc in tqdm(pool.imap(get_descriptors, smiles_df["canonical_smiles"]),
                                      total=len(smiles_df))]))
...
def preprocess_list(smiles, preprocess=True, randomize=True, features=False, n_jobs=1):
    df = pd.DataFrame(smiles)
    if preprocess:
        with Pool(n_jobs) as pool:
            df["canonical_smiles"] = [smi for smi in tqdm(pool.imap(preprocess_smiles, df[0]), total=len(df))]
    else:
        df["canonical_smiles"] = df[0]
    ...
    if randomize:
        with Pool(n_jobs) as pool:
            df["random_smiles"] = [smi for smi in tqdm(pool.imap(randomize_smile, df["canonical_smiles"]),
                                                       total=len(df))]
    # df["random_smiles"] = df["canonical_smiles"].map(randomize_smile)
    if features:
        df = create_feature_df(df, n_jobs=n_jobs)
    return df
